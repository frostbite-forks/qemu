...
static void pvideo_vga_invalidate(NV20State *d)
{
    ...
    NV20_DPRINTF("pvideo_vga_invalidate %d %d\n", y1, y2);
...
uint64_t pvideo_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    nv20_reg_log_read(NV_PVIDEO, addr, size, r);
    return r;
}
...
void pvideo_write(void *opaque, hwaddr addr, uint64_t val, unsigned int size)
{
    ...
    nv20_reg_log_write(NV_PVIDEO, addr, size, val);
