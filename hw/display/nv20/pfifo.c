...
static void pfifo_run_pusher(NV20State *d);
static uint32_t ramht_hash(NV20State *d, uint32_t handle);
static RAMHTEntry ramht_lookup(NV20State *d, uint32_t handle);
...
uint64_t pfifo_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    nv20_reg_log_read(NV_PFIFO, addr, size, r);
    return r;
}
...
void pfifo_write(void *opaque, hwaddr addr, uint64_t val, unsigned int size)
{
    ...
    nv20_reg_log_write(NV_PFIFO, addr, size, val);
...
static bool can_fifo_access(NV20State *d) {
 ...
 */
static bool is_flip_stall_complete(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;

    uint32_t s = pgraph_reg_r(pg, NV_PGRAPH_SURFACE);

    NV20_DPRINTF("flip stall read: %d, write: %d, modulo: %d\n",
...
static bool pfifo_stall_for_flip(NV20State *d)
{
...
static bool pfifo_puller_should_stall(NV20State *d)
{
...
static ssize_t pfifo_run_puller(NV20State *d, uint32_t method_entry,
...
static bool pfifo_pusher_should_stall(NV20State *d)
{
...
static void pfifo_run_pusher(NV20State *d)
{
        ...
        if (method_count) {
            ...
            *status &= ~NV_PFIFO_CACHE1_STATUS_LOW_MARK;
            ...
            if ((word & 0xe0000003) == 0x20000000) {
                ...
                NV20_DPRINTF("pb OLD_JMP 0x%x\n", dma_get_v);
            } else if ((word & 3) == 1) {
                /* jump */
                d->pfifo.regs[NV_PFIFO_CACHE1_DMA_GET_JMP_SHADOW] =
                    dma_get_v;
                dma_get_v = word & 0xfffffffc;
                NV20_DPRINTF("pb JMP 0x%x\n", dma_get_v);
            } else if ((word & 3) == 2) {
                /* call */
                if (subroutine_state) {
                    SET_MASK(*dma_state, NV_PFIFO_CACHE1_DMA_STATE_ERROR,
                ...
                *dma_dcount = 0;
                ...
                NV20_DPRINTF("pb reserved cmd 0x%x - 0x%x\n",
                             dma_get_v, word);
                SET_MASK(*dma_state, NV_PFIFO_CACHE1_DMA_STATE_ERROR,
    ...
    // NV20_DPRINTF("DMA pusher done: max 0x%" HWADDR_PRIx ", 0x%" HWADDR_PRIx " - 0x%" HWADDR_PRIx "\n",
    //      dma_len, control->dma_get, control->dma_put);

    uint32_t error = GET_MASK(*dma_state, NV_PFIFO_CACHE1_DMA_STATE_ERROR);
    if (error) {
        NV20_DPRINTF("pb error: %d\n", error);
