...
#ifndef HW_XBOX_NV20_PGRAPH_H
#define HW_XBOX_NV20_PGRAPH_H
...
typedef struct NV20State NV20State;
...
typedef struct PGRAPHRenderer {
    ...
    struct {
        ...
        void (*init)(NV20State *d, Error **errp);
        void (*finalize)(NV20State *d);
        void (*clear_report_value)(NV20State *d);
        void (*clear_surface)(NV20State *d, uint32_t parameter);
        void (*draw_begin)(NV20State *d);
        void (*draw_end)(NV20State *d);
        void (*flip_stall)(NV20State *d);
        void (*flush_draw)(NV20State *d);
        void (*get_report)(NV20State *d, uint32_t parameter);
        void (*image_blit)(NV20State *d);
        void (*pre_savevm_trigger)(NV20State *d);
        void (*pre_savevm_wait)(NV20State *d);
        void (*pre_shutdown_trigger)(NV20State *d);
        void (*pre_shutdown_wait)(NV20State *d);
        void (*process_pending)(NV20State *d);
        void (*process_pending_reports)(NV20State *d);
        void (*surface_flush)(NV20State *d);
        void (*surface_update)(NV20State *d, bool upload, bool color_write, bool zeta_write);
        void (*set_surface_scale_factor)(NV20State *d, unsigned int scale);
        unsigned int (*get_surface_scale_factor)(NV20State *d);
        int (*get_framebuffer_surface)(NV20State *d);
...
typedef struct PGRAPHState {
    ...
    hwaddr dma_a, dma_b;
    bool texture_dirty[NV20_MAX_TEXTURES];

    bool texture_matrix_enable[NV20_MAX_TEXTURES];
    ...
    uint32_t program_data[NV20_MAX_TRANSFORM_PROGRAM_LENGTH][VSH_TOKEN_SIZE];
    bool program_data_dirty;

    uint32_t vsh_constants[NV20_VERTEXSHADER_CONSTANTS][4];
    bool vsh_constants_dirty[NV20_VERTEXSHADER_CONSTANTS];

    /* lighting constant arrays */
    uint32_t ltctxa[NV20_LTCTXA_COUNT][4];
    bool ltctxa_dirty[NV20_LTCTXA_COUNT];
    uint32_t ltctxb[NV20_LTCTXB_COUNT][4];
    bool ltctxb_dirty[NV20_LTCTXB_COUNT];
    uint32_t ltc1[NV20_LTC1_COUNT][4];
    bool ltc1_dirty[NV20_LTC1_COUNT];

    float material_alpha;

    // should figure out where these are in lighting context
    float light_infinite_half_vector[NV20_MAX_LIGHTS][3];
    float light_infinite_direction[NV20_MAX_LIGHTS][3];
    float light_local_position[NV20_MAX_LIGHTS][3];
    float light_local_attenuation[NV20_MAX_LIGHTS][3];
    ...
    VertexAttribute vertex_attributes[NV20_VERTEXSHADER_ATTRIBUTES];
    uint16_t compressed_attrs;
    uint16_t uniform_attrs;
    uint16_t swizzle_attrs;

    unsigned int inline_array_length;
    uint32_t inline_array[NV20_MAX_BATCH_LENGTH];

    unsigned int inline_elements_length;
    uint32_t inline_elements[NV20_MAX_BATCH_LENGTH];
...
int pgraph_method(NV20State *d, unsigned int subchannel, unsigned int method,
...
{
    assert(stage < NV20_MAX_TEXTURES);
...
void pgraph_write_zpass_pixel_cnt_report(NV20State *d, uint32_t parameter, uint32_t result);
