...
uint64_t pgraph_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    nv20_reg_log_read(NV_PGRAPH, addr, size, r);
    return r;
}
...
void pgraph_write(void *opaque, hwaddr addr, uint64_t val, unsigned int size)
{
    ...
    nv20_reg_log_write(NV_PGRAPH, addr, size, val);
...
#endif
            NV20_DPRINTF("PGRAPH: read channel %d context from %" HWADDR_PRIx "\n",
...
void pgraph_context_switch(NV20State *d, unsigned int channel_id)
{
    PGRAPHState *pg = &d->pgraph;
    ...
    if (!valid) {
        ...
        NV20_DPRINTF("pgraph switching to ch %d\n", channel_id);
...
void pgraph_init(NV20State *d)
{
    ...
    for (int i = 0; i < NV20_VERTEXSHADER_ATTRIBUTES; i++) {
        ...
        attribute->inline_buffer = (float*)g_malloc(NV20_MAX_BATCH_LENGTH
                                              * sizeof(float) * 4);
...
#endif
    if (renderers[CONFIG_DISPLAY_RENDERER_NULL]) {
        return CONFIG_DISPLAY_RENDERER_NULL;
    }
    fprintf(stderr, "nv20-vga: Warning: No available renderer\n");
    return CONFIG_DISPLAY_RENDERER_NULL;
}
...
void pgraph_init_thread(NV20State *d)
{
...
int nv20_get_framebuffer_surface(void)
{
    NV20State *d = g_nv20;
    PGRAPHState *pg = &d->pgraph;
    int s = 0;
...
void nv20_release_framebuffer_surface(void)
{
    NV20State *d = g_nv20;
    PGRAPHState *pg = &d->pgraph;
    qemu_mutex_lock(&pg->renderer_lock);
...
void nv20_set_surface_scale_factor(unsigned int scale)
{
    NV20State *d = g_nv20;
...
unsigned int nv20_get_surface_scale_factor(void)
{
    NV20State *d = g_nv20;
    int s = 1;
                              ...
                              unsigned int method, uint32_t parameter)
{
    ...
    if (last == NV097_ARRAY_ELEMENT16 && method != last) {
        ...
        trace_nv20_pgraph_method_abbrev(subchannel, graphics_class, last,
                                        method_name, count);
    }
    ...
    if (method != NV097_ARRAY_ELEMENT16) {
        ...
        trace_nv20_pgraph_method(subchannel, graphics_class, method,
...
int pgraph_method(NV20State *d, unsigned int subchannel,
                   unsigned int method, uint32_t parameter,
...
unhandled:
    trace_nv20_pgraph_method_unhandled(subchannel, graphics_class,
                                           method, parameter);
    return num_processed;
}
...
DEF_METHOD(NV097, FLIP_INCREMENT_WRITE)
{
    ...
    trace_nv20_pgraph_flip_increment_write(old, new);
    pg->frame_time++;
}

DEF_METHOD(NV097, FLIP_STALL)
{
    trace_nv20_pgraph_flip_stall();
    d->pgraph.renderer->ops.surface_update(d, false, true, true);
    d->pgraph.renderer->ops.flip_stall(d);
    nv20_profile_flip_stall();
...
DEF_METHOD(NV097, SET_BLEND_FUNC_SFACTOR)
{
    ...
    default:
        NV20_DPRINTF("Unknown blend source factor: 0x%08x\n", parameter);
        return; /* discard */
    }
...
DEF_METHOD(NV097, SET_BLEND_FUNC_DFACTOR)
{
    ...
    default:
        NV20_DPRINTF("Unknown blend destination factor: 0x%08x\n", parameter);
        return; /* discard */
    }
...
DEF_METHOD(NV097, SET_BLEND_EQUATION)
{
    ...
    case NV097_SET_BLEND_EQUATION_V_FUNC_ADD_SIGNED:
        equation = 6; break;
    default:
        NV20_DPRINTF("Unknown blend equation: 0x%08x\n", parameter);
        return; /* discard */
    }
...
DEF_METHOD(NV097, SET_FRONT_FACE)
{
    ...
    case NV097_SET_FRONT_FACE_V_CCW:
        ccw = true; break;
    default:
        NV20_DPRINTF("Unknown front face: 0x%08x\n", parameter);
        return; /* discard */
    }
...
DEF_METHOD_INC(NV097, SET_TRANSFORM_PROGRAM)
{
    ...
    assert(program_load < NV20_MAX_TRANSFORM_PROGRAM_LENGTH);
...
DEF_METHOD_INC(NV097, SET_TRANSFORM_CONSTANT)
{
    ...
    assert(const_load < NV20_VERTEXSHADER_CONSTANTS);
...
DEF_METHOD_INC(NV097, SET_VERTEX3F)
{
    ...
    VertexAttribute *attribute =
        &pg->vertex_attributes[NV20_VERTEX_ATTR_POSITION];
    pgraph_allocate_inline_buffer_vertices(pg, NV20_VERTEX_ATTR_POSITION);
...
{
    VertexAttribute *attribute = &pg->vertex_attributes[NV20_VERTEX_ATTR_FOG];
    pgraph_allocate_inline_buffer_vertices(pg, NV20_VERTEX_ATTR_FOG);
...
{
    VertexAttribute *attribute = &pg->vertex_attributes[NV20_VERTEX_ATTR_WEIGHT];
    pgraph_allocate_inline_buffer_vertices(pg, NV20_VERTEX_ATTR_WEIGHT);
    attribute->inline_value[0] = *(float*)&parameter;
...
DEF_METHOD_INC(NV097, SET_NORMAL3S)
{
    ...
    VertexAttribute *attribute =
        &pg->vertex_attributes[NV20_VERTEX_ATTR_NORMAL];
    pgraph_allocate_inline_buffer_vertices(pg, NV20_VERTEX_ATTR_NORMAL);
...
{
    SET_VERTEX_ATTRIBUTE_4S(NV097_SET_TEXCOORD0_4S, NV20_VERTEX_ATTR_TEXTURE0);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD1_4S)
{
    SET_VERTEX_ATTRIBUTE_4S(NV097_SET_TEXCOORD1_4S, NV20_VERTEX_ATTR_TEXTURE1);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD2_4S)
{
    SET_VERTEX_ATTRIBUTE_4S(NV097_SET_TEXCOORD2_4S, NV20_VERTEX_ATTR_TEXTURE2);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD3_4S)
{
    SET_VERTEX_ATTRIBUTE_4S(NV097_SET_TEXCOORD3_4S, NV20_VERTEX_ATTR_TEXTURE3);
}
...
{
    SET_VERTEX_ATRIBUTE_TEX_2S(NV20_VERTEX_ATTR_TEXTURE0);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD1_2S)
{
    SET_VERTEX_ATRIBUTE_TEX_2S(NV20_VERTEX_ATTR_TEXTURE1);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD2_2S)
{
    SET_VERTEX_ATRIBUTE_TEX_2S(NV20_VERTEX_ATTR_TEXTURE2);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD3_2S)
{
    SET_VERTEX_ATRIBUTE_TEX_2S(NV20_VERTEX_ATTR_TEXTURE3);
}
...
{
    SET_VERTEX_COLOR_3F(NV097_SET_DIFFUSE_COLOR3F, NV20_VERTEX_ATTR_DIFFUSE);
}

DEF_METHOD_INC(NV097, SET_SPECULAR_COLOR3F)
{
    SET_VERTEX_COLOR_3F(NV097_SET_SPECULAR_COLOR3F, NV20_VERTEX_ATTR_SPECULAR);
}
...
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_NORMAL3F, NV20_VERTEX_ATTR_NORMAL);
}

DEF_METHOD_INC(NV097, SET_DIFFUSE_COLOR4F)
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_DIFFUSE_COLOR4F, NV20_VERTEX_ATTR_DIFFUSE);
}

DEF_METHOD_INC(NV097, SET_SPECULAR_COLOR4F)
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_SPECULAR_COLOR4F,
                           NV20_VERTEX_ATTR_SPECULAR);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD0_4F)
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_TEXCOORD0_4F, NV20_VERTEX_ATTR_TEXTURE0);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD1_4F)
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_TEXCOORD1_4F, NV20_VERTEX_ATTR_TEXTURE1);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD2_4F)
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_TEXCOORD2_4F, NV20_VERTEX_ATTR_TEXTURE2);
}

DEF_METHOD_INC(NV097, SET_TEXCOORD3_4F)
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_TEXCOORD3_4F, NV20_VERTEX_ATTR_TEXTURE3);
}

DEF_METHOD_INC(NV097, SET_WEIGHT4F)
{
    SET_VERTEX_ATTRIBUTE_F(NV097_SET_WEIGHT4F, NV20_VERTEX_ATTR_WEIGHT);
}
...
DEF_METHOD_INC(NV097, SET_WEIGHT2F)
{
    ...
    VertexAttribute *attribute =
        &pg->vertex_attributes[NV20_VERTEX_ATTR_WEIGHT];
    pgraph_allocate_inline_buffer_vertices(pg, NV20_VERTEX_ATTR_WEIGHT);
    attribute->inline_value[slot] = *(float*)&parameter;
    attribute->inline_value[2] = 0.0f;
    attribute->inline_value[3] = 1.0f;
...
DEF_METHOD_INC(NV097, SET_VERTEX_DATA_ARRAY_FORMAT)
{
    ...
    NV20_DPRINTF("vertex data array format=%d, count=%d, stride=%d\n",
...
DEF_METHOD(NV097, SET_BEGIN_END)
{
    if (parameter == NV097_SET_BEGIN_END_OP_END) {
        if (pg->primitive_mode == PRIM_TYPE_INVALID) {
            NV20_DPRINTF("End without Begin!\n");
            pgraph_reset_inline_buffers(pg);
            return;
        }
        nv20_profile_inc_counter(NV20_PROF_BEGIN_ENDS);
        d->pgraph.renderer->ops.draw_end(d);
        pgraph_reset_inline_buffers(pg);
        pg->primitive_mode = PRIM_TYPE_INVALID;
    } else {
        if (pg->primitive_mode != PRIM_TYPE_INVALID) {
            NV20_DPRINTF("Begin without End!\n");
            return;
        }
...
static void pgraph_expand_draw_arrays(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    ...
    assert((pg->inline_elements_length + count) < NV20_MAX_BATCH_LENGTH);
...
{
    if (pg->primitive_mode == PRIM_TYPE_INVALID) {
        NV20_DPRINTF("Vertex data being sent outside of begin/end block!\n");
    }
}
...
DEF_METHOD_NON_INC(NV097, ARRAY_ELEMENT16)
{
    ...
    assert(pg->inline_elements_length < NV20_MAX_BATCH_LENGTH);
...
DEF_METHOD(NV097, DRAW_ARRAYS)
{
    ...
    if (pg->inline_elements_length) {
         ...
         * NV20_MAX_BATCH_LENGTH (which must be larger to accommodate
...
{
    pgraph_check_within_begin_end_block(pg);
    assert(pg->inline_array_length < NV20_MAX_BATCH_LENGTH);
...
DEF_METHOD(NV097, LAUNCH_TRANSFORM_PROGRAM)
{
    ...
    assert(program_start < NV20_MAX_TRANSFORM_PROGRAM_LENGTH);
    Nv2aVshProgram program;
    Nv2aVshParseResult result = nv2a_vsh_parse_program(
            &program,
            pg->program_data[program_start],
            NV20_MAX_TRANSFORM_PROGRAM_LENGTH - program_start);
...
{
    assert(parameter < NV20_MAX_TRANSFORM_PROGRAM_LENGTH);
    PG_SET_MASK(NV_PGRAPH_CHEOPS_OFFSET,
...
{
    assert(parameter < NV20_VERTEXSHADER_CONSTANTS);
    PG_SET_MASK(NV_PGRAPH_CHEOPS_OFFSET,
...
void pgraph_write_zpass_pixel_cnt_report(NV20State *d, uint32_t parameter,
                                         uint32_t result)
{
    PGRAPHState *pg = &d->pgraph;
    ...
    NV20_DPRINTF("Report result %d @%" HWADDR_PRIx, result, offset);
}

static void do_wait_for_renderer_switch(CPUState *cpu, run_on_cpu_data data)
{
    NV20State *d = (NV20State *)data.host_ptr;
...
void pgraph_process_pending(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
...
void pgraph_process_pending_reports(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    pg->renderer->ops.process_pending_reports(d);
}

void pgraph_pre_savevm_trigger(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    pg->renderer->ops.pre_savevm_trigger(d);
}

void pgraph_pre_savevm_wait(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    pg->renderer->ops.pre_savevm_wait(d);
}

void pgraph_pre_shutdown_trigger(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    pg->renderer->ops.pre_shutdown_trigger(d);
}

void pgraph_pre_shutdown_wait(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
