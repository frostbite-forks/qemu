...
#include "hw/display/nv20/xemu-version.h"
                               ...
                               const char *name)
{
    ...
    NV20_GL_DGROUP_BEGIN("Creating new %s", name);

    NV20_DPRINTF("compile new %s, code:\n%s\n", name, code);
...
bool pgraph_gl_shader_load_from_memory(ShaderBinding *binding)
{
    ...
    if (!valid) {
        ...
        NV20_DPRINTF("failed to load shader binary from disk: %s\n", log);
        glDeleteProgram(gl_program);
        return false;
    }
...
void pgraph_gl_bind_shaders(PGRAPHState *pg)
{
    ...
    NV20_GL_DGROUP_BEGIN("%s (%s)", __func__,
