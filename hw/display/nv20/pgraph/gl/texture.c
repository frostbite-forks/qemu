...
void pgraph_gl_mark_textures_possibly_dirty(NV20State *d,
...
void pgraph_gl_bind_textures(NV20State *d)
{
    ...
    NV20_GL_DGROUP_BEGIN("%s", __func__);

    for (i=0; i<NV20_MAX_TEXTURES; i++) {
        ...
        if (filter & NV_PGRAPH_TEXFILTER0_ASIGNED) NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_ASIGNED");
        if (filter & NV_PGRAPH_TEXFILTER0_RSIGNED) NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_RSIGNED");
        if (filter & NV_PGRAPH_TEXFILTER0_GSIGNED) NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_GSIGNED");
        if (filter & NV_PGRAPH_TEXFILTER0_BSIGNED) NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_BSIGNED");
        ...
        assert((texture_vram_offset + length) < memory_region_size(&d->vram_mr));
                                        ...
                                        const uint8_t *palette_data)
{
    ...
    if (s.cubemap) {
        ...
        if (f.linear) {
            ...
            default:
                NV20_GL_DPRINTF(true, "Invalid texture dimensionality: %u", (unsigned)s.dimensionality);
    ...
    NV20_GL_DLABEL(GL_TEXTURE, gl_texture,
                   "offset: 0x%08lx, format: 0x%02X%s, %d dimensions%s, "
                   "width: %d, height: %d, depth: %d",
                   texture_data - g_nv20->vram_ptr,
    ...
    if (gl_target == GL_TEXTURE_CUBE_MAP) {
        ...
        length = (length + NV20_CUBEMAP_FACE_ALIGNMENT - 1) & ~(NV20_CUBEMAP_FACE_ALIGNMENT - 1);
