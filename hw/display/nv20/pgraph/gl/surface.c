...
static void surface_download_to_buffer(NV20State *d, SurfaceBinding *surface,
...
void pgraph_gl_set_surface_scale_factor(NV20State *d, unsigned int scale)
{
...
unsigned int pgraph_gl_get_surface_scale_factor(NV20State *d)
{
                                            ...
                                            TextureShape *shape)
{
    ...
    trace_nv20_pgraph_surface_texture_compat_failed(
        surface_fmt, texture_fmt);
    return false;
}

static void render_surface_to(NV20State *d, SurfaceBinding *surface,
...
static void render_surface_to_texture_slow(NV20State *d,
                                           ...
                                           int texture_unit)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_SURF_TO_TEX_FALLBACK);
 ...
 * couple of items.
 */
void pgraph_gl_render_surface_to_texture(NV20State *d, SurfaceBinding *surface,
                                      TextureBinding *texture,
                                      TextureShape *texture_shape,
                                      int texture_unit)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_SURF_TO_TEX);
                                    ...
                                    hwaddr len, bool write)
{
    ...
    QTAILQ_FOREACH(surface, &r->surfaces, entry) {
        ...
        if (write) {
            trace_nv20_pgraph_surface_cpu_write(surface->vram_addr, offset);
        } else {
            trace_nv20_pgraph_surface_cpu_read(surface->vram_addr, offset);
        }
...
static void register_cpu_access_callback(NV20State *d, SurfaceBinding *surface)
{
    if (tcg_enabled()) {
        if (surface->width && surface->height) {
            surface->access_cb = mem_access_callback_insert(
                qemu_get_cpu(0), &d->vram_mr, surface->vram_addr, surface->size,
...
static void unregister_cpu_access_callback(NV20State *d,
...
static void invalidate_overlapping_surfaces(NV20State *d, SurfaceBinding *surface)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
    ...
    QTAILQ_FOREACH_SAFE(other_surface, &r->surfaces, entry, next_surface) {
        if (check_surfaces_overlap(surface, other_surface)) {
            trace_nv20_pgraph_surface_evict_overlapping(
...
static SurfaceBinding *surface_put(NV20State *d, hwaddr addr,
...
SurfaceBinding *pgraph_gl_surface_get(NV20State *d, hwaddr addr)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
SurfaceBinding *pgraph_gl_surface_get_within(NV20State *d, hwaddr addr)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
void pgraph_gl_surface_invalidate(NV20State *d, SurfaceBinding *surface)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;

    trace_nv20_pgraph_surface_invalidated(surface->vram_addr);
...
static void surface_evict_old(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
    ...
    QTAILQ_FOREACH_SAFE(s, &r->surfaces, entry, next) {
        ...
        if (last_used >= surface_age_limit) {
            trace_nv20_pgraph_surface_evict_reason("old", s->vram_addr);
...
void pgraph_gl_surface_download_if_dirty(NV20State *d,
                                           SurfaceBinding *surface)
{
    if (surface->draw_dirty) {
...
static void bind_current_surface(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
                                       ...
                                       uint8_t *pixels)
{
    ...
    trace_nv20_pgraph_surface_download(
...
static void surface_download(NV20State *d, SurfaceBinding *surface, bool force)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_SURF_DOWNLOAD);

    surface_download_to_buffer(d, surface, true, false, true,
                               d->vram_ptr + surface->vram_addr);

    memory_region_set_client_dirty(&d->vram_mr, surface->vram_addr,
...
void pgraph_gl_process_pending_downloads(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
void pgraph_gl_download_dirty_surfaces(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
void pgraph_gl_upload_surface_data(NV20State *d, SurfaceBinding *surface,
                                bool force)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_SURF_UPLOAD);

    trace_nv20_pgraph_surface_upload(
                 surface->color ? "COLOR" : "ZETA",
                 surface->swizzle ? "sz" : "lin", surface->vram_addr,
                 surface->width, surface->height, surface->pitch,
                 surface->fmt.bytes_per_pixel);
...
static void compare_surfaces(SurfaceBinding *s1, SurfaceBinding *s2)
{
    #define DO_CMP(fld) \
        if (s1->fld != s2->fld) \
            trace_nv20_pgraph_surface_compare_mismatch( \
...
static void populate_surface_binding_entry_sized(NV20State *d, bool color,
                                                 unsigned int width,
                                                 unsigned int height,
...
static void populate_surface_binding_entry(NV20State *d, bool color,
                                                  SurfaceBinding *entry)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
static void update_surface_part(NV20State *d, bool upload, bool color)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
    ...
    bool mem_dirty = !tcg_enabled() && memory_region_test_and_clear_dirty(
                                           &d->vram_mr, entry.vram_addr, entry.size,
                                           DIRTY_MEMORY_NV2A);
    ...
    if (upload && (surface->buffer_dirty || mem_dirty)) {
        ...
        if (found != NULL) {
            ...
            SurfaceBinding *other = (color ? r->zeta_binding
                                           : r->color_binding);
            if (found == other) {
                NV20_UNIMPLEMENTED("Same color & zeta surface offset");
                pgraph_gl_unbind_surface(d, !color);
            }
        }

        trace_nv20_pgraph_surface_target(
...
#define TRACE_ARGS found->vram_addr, found->width, found->height, \
            ...
            if (found->color) {
                trace_nv20_pgraph_surface_match_color(TRACE_ARGS);
            } else {
                trace_nv20_pgraph_surface_match_zeta(TRACE_ARGS);
            }
#undef TRACE_ARGS
            ...
            if (found->swizzle != entry.swizzle) {
                ...
                if (is_compatible) {
                    trace_nv20_pgraph_surface_migrate_type(
            ...
            if (is_compatible) {
                ...
                trace_nv20_pgraph_surface_evict_reason(
        ...
        if (should_create) {
            ...
            NV20_GL_DLABEL(GL_TEXTURE, entry.gl_buffer,
...
#define TRACE_ARGS found->vram_addr, found->width, found->height, \
        ...
        if (color) {
            if (should_create) {
                trace_nv20_pgraph_surface_create_color(TRACE_ARGS);
            } else {
                trace_nv20_pgraph_surface_hit_color(TRACE_ARGS);
            }

            r->color_binding = found;
        } else {
            if (should_create) {
                trace_nv20_pgraph_surface_create_zeta(TRACE_ARGS);
            } else {
                trace_nv20_pgraph_surface_hit_zeta(TRACE_ARGS);
            }
...
void pgraph_gl_unbind_surface(NV20State *d, bool color)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
void pgraph_gl_surface_update(NV20State *d, bool upload, bool color_write,
                           bool zeta_write)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
static void flush_surfaces(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
void pgraph_gl_surface_flush(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
