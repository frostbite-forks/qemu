...
GloContext *g_nv20_context_render;
GloContext *g_nv20_context_display;

static void early_context_init(void)
{
    g_nv20_context_render = glo_context_create();
    g_nv20_context_display = glo_context_create();
    ...
    glo_set_current(g_nv20_context_display);
}

static void pgraph_gl_init(NV20State *d, Error **errp)
{
    ...
    /* fire up opengl */
    glo_set_current(g_nv20_context_render);
...
static void pgraph_gl_finalize(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
...
static void pgraph_gl_flip_stall(NV20State *d)
{
    NV20_GL_DFRAME_TERMINATOR();
    glFinish();
}

static void pgraph_gl_flush(NV20State *d)
{
    pgraph_gl_surface_flush(d);
    pgraph_gl_mark_textures_possibly_dirty(d, 0, memory_region_size(&d->vram_mr));
    pgraph_gl_update_entire_memory_buffer(d);
    /* FIXME: Flush more? */
...
static void pgraph_gl_process_pending(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
static void pgraph_gl_pre_savevm_trigger(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
static void pgraph_gl_pre_savevm_wait(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
static void pgraph_gl_pre_shutdown_trigger(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
...
static void pgraph_gl_pre_shutdown_wait(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHGLState *r = pg->gl_renderer_state;
