...
void pgraph_gl_update_entire_memory_buffer(NV20State *d)
{
    ...
    glBindBuffer(GL_ARRAY_BUFFER, r->gl_memory_buffer);
    glBufferSubData(GL_ARRAY_BUFFER, 0, memory_region_size(&d->vram_mr), d->vram_ptr);
}

void pgraph_gl_bind_vertex_attributes(NV20State *d, unsigned int min_element,
                                   ...
                                   unsigned int provoking_element)
{
    ...
    if (inline_data) {
        NV20_GL_DGROUP_BEGIN("%s (num_elements: %d inline stride: %d)",
                             __func__, num_elements, inline_stride);
    } else {
        NV20_GL_DGROUP_BEGIN("%s (num_elements: %d)", __func__, num_elements);
    }
    ...
    for (int i = 0; i < NV20_VERTEXSHADER_ATTRIBUTES; i++) {
        ...
        nv20_profile_inc_counter(NV20_PROF_ATTR_BIND);
...
void pgraph_gl_init_buffers(NV20State *d)
{
    ...
    assert(max_vertex_attributes >= NV20_VERTEXSHADER_ATTRIBUTES);

    glGenBuffers(NV20_VERTEXSHADER_ATTRIBUTES, r->gl_inline_buffer);
    glGenBuffers(1, &r->gl_inline_array_buffer);

    glGenBuffers(1, &r->gl_memory_buffer);
    glBindBuffer(GL_ARRAY_BUFFER, r->gl_memory_buffer);
    glBufferData(GL_ARRAY_BUFFER, memory_region_size(&d->vram_mr),
...
void pgraph_gl_finalize_buffers(PGRAPHState *pg)
{
    ...
    glDeleteBuffers(NV20_VERTEXSHADER_ATTRIBUTES, r->gl_inline_buffer);
