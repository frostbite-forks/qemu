                                      ...
                                      unsigned int provoking_element)
{
    ...
    if (inline_data) {
        NV20_VK_DGROUP_BEGIN("%s (num_elements: %d inline stride: %d)",
                             __func__, num_elements, inline_stride);
    } else {
        NV20_VK_DGROUP_BEGIN("%s (num_elements: %d)", __func__, num_elements);
    }
    ...
    for (int i = 0; i < NV20_VERTEXSHADER_ATTRIBUTES; i++) {
        ...
        NV20_VK_DGROUP_BEGIN("[attr %02d] format=%s, count=%d, stride=%d", i,
        ...
        if (!attr->count) {
            ...
            NV20_VK_DPRINTF("inline_value = {%f, %f, %f, %f}",
        ...
        NV20_VK_DPRINTF("offset = %08" HWADDR_PRIx, attrib_data_addr);
