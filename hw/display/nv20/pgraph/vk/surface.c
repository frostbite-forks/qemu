...
void pgraph_vk_set_surface_scale_factor(NV20State *d, unsigned int scale)
{
...
unsigned int pgraph_vk_get_surface_scale_factor(NV20State *d)
{
    ...
    QTAILQ_FOREACH(surface, &r->surfaces, entry) {
        if (check_surface_overlaps_range(surface, start, size)) {
            pgraph_vk_surface_download_if_dirty(
                container_of(pg, NV20State, pgraph), surface);
        }
    }
}

static void download_surface_to_buffer(NV20State *d, SurfaceBinding *surface,
                                       uint8_t *pixels)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_QUEUE_SUBMIT_1);
    ...
    if (surface->swizzle) {
        ...
        nv20_profile_inc_counter(NV20_PROF_SURF_SWIZZLE);
        g_free(swizzle_buf);
    }
}

static void download_surface(NV20State *d, SurfaceBinding *surface, bool force)
{
...
void pgraph_vk_process_pending_downloads(NV20State *d)
{
...
void pgraph_vk_download_dirty_surfaces(NV20State *d)
{
    PGRAPHVkState *r = d->pgraph.vk_renderer_state;
...
static void unbind_surface(NV20State *d, bool color)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;
...
static void invalidate_surface(NV20State *d, SurfaceBinding *surface)
{
    PGRAPHVkState *r = d->pgraph.vk_renderer_state;
...
static void invalidate_overlapping_surfaces(NV20State *d,
                                            SurfaceBinding const *surface)
{
    PGRAPHVkState *r = d->pgraph.vk_renderer_state;
...
static void surface_put(NV20State *d, SurfaceBinding *surface)
{
    PGRAPHVkState *r = d->pgraph.vk_renderer_state;
...
SurfaceBinding *pgraph_vk_surface_get(NV20State *d, hwaddr addr)
{
    PGRAPHVkState *r = d->pgraph.vk_renderer_state;
...
SurfaceBinding *pgraph_vk_surface_get_within(NV20State *d, hwaddr addr)
{
    PGRAPHVkState *r = d->pgraph.vk_renderer_state;
...
static void create_surface_image(PGRAPHState *pg, SurfaceBinding *surface)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_QUEUE_SUBMIT_3);
    pgraph_vk_end_debug_marker(r, cmd);
    pgraph_vk_end_single_time_commands(pg, cmd);
    nv20_profile_inc_counter(NV20_PROF_SURF_CREATE);
}
...
static void expire_old_surfaces(NV20State *d)
{
    PGRAPHVkState *r = d->pgraph.vk_renderer_state;
...
void pgraph_vk_surface_download_if_dirty(NV20State *d, SurfaceBinding *surface)
{
    if (surface->draw_dirty) {
...
void pgraph_vk_upload_surface_data(NV20State *d, SurfaceBinding *surface,
                                   bool force)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;
    ...
    nv20_profile_inc_counter(NV20_PROF_QUEUE_SUBMIT_2);
    pgraph_vk_end_debug_marker(r, cmd);
    pgraph_vk_end_single_time_commands(pg, cmd);
...
static void populate_surface_binding_target_sized(NV20State *d, bool color,
                                                  unsigned int width,
                                                  unsigned int height,
                                                  SurfaceBinding *target)
{
...
static void populate_surface_binding_target(NV20State *d, bool color,
                                            SurfaceBinding *target)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;
    ...
    if (!current_binding ||
        ...
        if (surface != NULL) {
            ...
            void (*trace_fn)(uint32_t addr, uint32_t width, uint32_t height,
                             ...
                             uint32_t pitch) =
                surface->color ? trace_nv20_pgraph_surface_match_color :
                               trace_nv20_pgraph_surface_match_zeta;
        ...
        void (*trace_fn)(uint32_t addr, uint32_t width, uint32_t height,
            ...
            color ? (should_create ? trace_nv20_pgraph_surface_create_color :
                                     trace_nv20_pgraph_surface_hit_color) :
                    (should_create ? trace_nv20_pgraph_surface_create_zeta :
                                     trace_nv20_pgraph_surface_hit_zeta);
        trace_fn(surface->vram_addr, surface->width, surface->height,
                 surface->swizzle ? "sz" : "ln", surface->shape.anti_aliasing,
                 surface->shape.clip_x, surface->shape.clip_width,
...
// FIXME: Move to common?
void pgraph_vk_surface_update(NV20State *d, bool upload, bool color_write,
                              bool zeta_write)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;
...
{
    pgraph_vk_surface_flush(container_of(pg, NV20State, pgraph));
}

void pgraph_vk_surface_flush(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;
