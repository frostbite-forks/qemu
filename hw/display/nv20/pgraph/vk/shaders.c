...
const size_t MAX_UNIFORM_ATTR_VALUES_SIZE = NV20_VERTEXSHADER_ATTRIBUTES * 4 * sizeof(float);
...
static void create_descriptor_pool(PGRAPHState *pg)
{
    ...
    VkDescriptorPoolSize pool_sizes[] = {
        ...
        },
        {
            ...
            .descriptorCount = NV20_MAX_TEXTURES * num_sets,
        }
    };
...
static void create_descriptor_set_layout(PGRAPHState *pg)
{
    ...
    VkDescriptorSetLayoutBinding bindings[2 + NV20_MAX_TEXTURES];
...
void pgraph_vk_update_descriptor_sets(PGRAPHState *pg)
{
    ...
    VkWriteDescriptorSet descriptor_writes[2 + NV20_MAX_TEXTURES];
    ...
    VkDescriptorImageInfo image_infos[NV20_MAX_TEXTURES];
    for (int i = 0; i < NV20_MAX_TEXTURES; i++) {
...
static void shader_cache_entry_init(Lru *lru, LruNode *node, const void *state)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_SHADER_GEN);
                                                   ...
                                                   const ShaderState *state)
{
    ...
    ShaderBinding *binding = container_of(node, ShaderBinding, node);
    NV20_VK_DPRINTF("shader state hash: %016" PRIx64 " %p", hash, binding);
    return binding;
}
...
static void update_shader_uniforms(PGRAPHState *pg)
{
    ...
    PGRAPHVkState *r = pg->vk_renderer_state;
    nv20_profile_inc_counter(NV20_PROF_SHADER_BIND);
    ...
    nv20_profile_inc_counter(r->uniforms_changed ?
                                 NV20_PROF_SHADER_UBO_DIRTY :
                                 NV20_PROF_SHADER_UBO_NOTDIRTY);
...
void pgraph_vk_bind_shaders(PGRAPHState *pg)
{
    ...
    if (!r->shader_binding ||
        ...
        nv20_profile_inc_counter(NV20_PROF_SHADER_BIND_NOTDIRTY);
    }
