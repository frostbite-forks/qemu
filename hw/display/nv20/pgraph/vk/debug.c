...
int nv20_vk_dgroup_indent = 0;

void pgraph_vk_debug_init(void)
{
#ifdef CONFIG_RENDERDOC
    nv20_dbg_renderdoc_init();
#endif
}
...
{
#ifdef CONFIG_RENDERDOC
    if (nv20_dbg_renderdoc_available()) {
        RENDERDOC_API_1_6_0 *rdoc_api = nv20_dbg_renderdoc_get_api();

        PGRAPHVkState *r = g_nv20->pgraph.vk_renderer_state;
        if (rdoc_api->IsTargetControlConnected()) {
            ...
            if (capturing && renderdoc_capture_frames == 0) {
                ...
                if (renderdoc_trace_frames) {
                    trace_enable_events("-nv20_pgraph_*");
