...
static size_t get_cubemap_layer_size(PGRAPHState *pg, TextureShape s)
{
    ...
    return ROUND_UP(length, NV20_CUBEMAP_FACE_ALIGNMENT);
}
...
static TextureLayout *get_texture_layout(PGRAPHState *pg, int texture_idx)
{
    ...
    NV20_VK_DGROUP_BEGIN("Texture %d: cubemap=%d, dimensionality=%d, color_format=0x%x, levels=%d, width=%d, height=%d, depth=%d border=%d, min_mipmap_level=%d, max_mipmap_level=%d, pitch=%d",
        texture_idx,
        s.cubemap,
        s.dimensionality,
        s.color_format,
    ...
    if (s.dimensionality == 2) {
        ...
        for (int layer = 0; layer < num_layers; layer++) {
            ...
            for (int level = 0; level < s.levels; level++) {
                NV20_VK_DPRINTF("Layer %d Level %d @ %x", layer, level, (int)((char*)texture_data_ptr - (char*)d->vram_ptr));
...
void pgraph_vk_mark_textures_possibly_dirty(NV20State *d,
...
static bool check_texture_dirty(NV20State *d, hwaddr addr, hwaddr size)
{
    hwaddr end = TARGET_PAGE_ALIGN(addr + size);
    addr &= TARGET_PAGE_MASK;
    assert(end < memory_region_size(&d->vram_mr));
    return memory_region_test_and_clear_dirty(&d->vram_mr, addr, end - addr,
                                              DIRTY_MEMORY_VGA);
}

// Check if any of the pages spanned by the a texture are dirty.
static bool check_texture_possibly_dirty(NV20State *d,
                                 ...
                                 TextureBinding *binding)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_TEX_UPLOAD);
    ...
    for (int layer_idx = 0; layer_idx < num_layers; layer_idx++) {
        TextureLayer *layer = &layout->layers[layer_idx];
        NV20_VK_DPRINTF("Layer %d", layer_idx);
        for (int level_idx = 0; level_idx < state->levels; level_idx++) {
            TextureLevel *level = &layer->levels[level_idx];
            NV20_VK_DPRINTF(" - Level %d, w=%d h=%d d=%d @ %08" HWADDR_PRIx,
    ...
    nv20_profile_inc_counter(NV20_PROF_QUEUE_SUBMIT_4);
                                         ...
                                         TextureBinding *texture)
{
    ...
    trace_nv20_pgraph_surface_render_to_texture(
...
{
    NV20_VK_DGROUP_BEGIN("Creating texture %d", texture_idx);
    ...
    if (filter & NV_PGRAPH_TEXFILTER0_ASIGNED)
        NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_ASIGNED");
    if (filter & NV_PGRAPH_TEXFILTER0_RSIGNED)
        NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_RSIGNED");
    if (filter & NV_PGRAPH_TEXFILTER0_GSIGNED)
        NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_GSIGNED");
    if (filter & NV_PGRAPH_TEXFILTER0_BSIGNED)
        NV20_UNIMPLEMENTED("NV_PGRAPH_TEXFILTER0_BSIGNED");
...
{
    NV20_VK_DGROUP_BEGIN("%s", __func__);
...
void pgraph_vk_trim_texture_cache(PGRAPHState *pg)
{
    ...
    NV20_VK_DPRINTF("Evicted %d textures, %d remain", num_evicted, r->texture_cache.num_used);
}
