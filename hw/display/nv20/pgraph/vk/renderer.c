...
static void pgraph_vk_init(NV20State *d, Error **errp)
{
...
static void pgraph_vk_finalize(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
...
static void pgraph_vk_flush(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;

    pgraph_vk_finish(pg, VK_FINISH_REASON_FLUSH);
    pgraph_vk_surface_flush(d);
    pgraph_vk_mark_textures_possibly_dirty(d, 0, memory_region_size(&d->vram_mr));
    pgraph_vk_update_vertex_ram_buffer(&d->pgraph, 0, d->vram_ptr,
                                       memory_region_size(&d->vram_mr));
    for (int i = 0; i < 4; i++) {
...
static void pgraph_vk_sync(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
...
static void pgraph_vk_process_pending(NV20State *d)
{
...
static void pgraph_vk_flip_stall(NV20State *d)
{
    pgraph_vk_finish(&d->pgraph, VK_FINISH_REASON_FLIP_STALL);
    pgraph_vk_debug_frame_terminator();
}

static void pgraph_vk_pre_savevm_trigger(NV20State *d)
{
...
static void pgraph_vk_pre_savevm_wait(NV20State *d)
{
    qemu_event_wait(&d->pgraph.vk_renderer_state->dirty_surfaces_download_complete);
}

static void pgraph_vk_pre_shutdown_trigger(NV20State *d)
{
...
static void pgraph_vk_pre_shutdown_wait(NV20State *d)
{
    // qemu_event_wait(&d->pgraph.vk_renderer_state->shader_cache_writeback_complete);   
}

static int pgraph_vk_get_framebuffer_surface(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
...
void pgraph_vk_check_memory_budget(PGRAPHState *pg)
{
    ...
    for (int i = 0; i < props->memoryHeapCount; i++) {
        ...
        NV20_VK_DPRINTF("Heap %d: used %lu/%lu MiB (%.2f%%)", i,
