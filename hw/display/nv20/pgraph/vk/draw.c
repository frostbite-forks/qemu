...
void pgraph_vk_draw_begin(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;

    NV20_VK_DPRINTF("NV097_SET_BEGIN_END: 0x%x", d->pgraph.primitive_mode);
...
{
    NV20_VK_DPRINTF("Creating render pass");
...
static void create_frame_buffer(PGRAPHState *pg)
{
    ...
    NV20_VK_DPRINTF("Creating framebuffer");
...
{
    NV20_VK_DPRINTF("Destroying framebuffer");
    PGRAPHVkState *r = pg->vk_renderer_state;
...
static void create_clear_pipeline(PGRAPHState *pg)
{
    ...
    NV20_VK_DGROUP_BEGIN("Creating clear pipeline");
    ...
    NV20_VK_DPRINTF("Cache miss");
    nv20_profile_inc_counter(NV20_PROF_PIPELINE_GEN);
...
static bool check_pipeline_dirty(PGRAPHState *pg)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_PIPELINE_NOTDIRTY);
...
{
    NV20_VK_DGROUP_BEGIN("Creating pipeline");
...
static void push_vertex_attr_values(PGRAPHState *pg)
{
    ...
    float values[NV20_VERTEXSHADER_ATTRIBUTES][4];
...
static void begin_query(PGRAPHVkState *r)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_QUERY);
...
static void begin_render_pass(PGRAPHState *pg)
{
    ...
    nv20_profile_inc_counter(NV20_PROF_PIPELINE_RENDERPASSES);
...
const enum NV20_PROF_COUNTERS_ENUM finish_reason_to_counter_enum[] = {
    [VK_FINISH_REASON_VERTEX_BUFFER_DIRTY] = NV20_PROF_FINISH_VERTEX_BUFFER_DIRTY,
    [VK_FINISH_REASON_SURFACE_CREATE] = NV20_PROF_FINISH_SURFACE_CREATE,
    [VK_FINISH_REASON_SURFACE_DOWN] = NV20_PROF_FINISH_SURFACE_DOWN,
    [VK_FINISH_REASON_NEED_BUFFER_SPACE] = NV20_PROF_FINISH_NEED_BUFFER_SPACE,
    [VK_FINISH_REASON_FRAMEBUFFER_DIRTY] = NV20_PROF_FINISH_FRAMEBUFFER_DIRTY,
    [VK_FINISH_REASON_PRESENTING] = NV20_PROF_FINISH_PRESENTING,
    [VK_FINISH_REASON_FLIP_STALL] = NV20_PROF_FINISH_FLIP_STALL,
    [VK_FINISH_REASON_FLUSH] = NV20_PROF_FINISH_FLUSH,
    [VK_FINISH_REASON_STALLED] = NV20_PROF_FINISH_STALLED,
};
...
void pgraph_vk_finish(PGRAPHState *pg, FinishReason finish_reason)
{
    ...
    if (r->in_command_buffer) {
        nv20_profile_inc_counter(finish_reason_to_counter_enum[finish_reason]);
        ...
        nv20_profile_inc_counter(NV20_PROF_QUEUE_SUBMIT);
...
static void begin_draw(PGRAPHState *pg)
{
    ...
    if (must_bind_pipeline) {
        nv20_profile_inc_counter(NV20_PROF_PIPELINE_BIND);
...
void pgraph_vk_draw_end(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;
...
static void sync_vertex_ram_buffer(PGRAPHState *pg)
{
    ...
    NV20_VK_DGROUP_BEGIN("Sync vertex RAM buffer");

    for (int i = 0; i < r->num_vertex_ram_buffer_syncs; i++) {
        NV20_VK_DPRINTF("Need to sync vertex memory @%" HWADDR_PRIx
                        ", %" HWADDR_PRIx " bytes",
        ...
        NV20_VK_DPRINTF("- %d: %08" HWADDR_PRIx " %zd bytes"
    ...
    if (num_syncs < r->num_vertex_ram_buffer_syncs) {
        NV20_VK_DPRINTF("Reduced to %d sync checks", num_syncs);
    }
    ...
    for (int i = 0; i < num_syncs; i++) {
        ...
        NV20_VK_DPRINTF("- %d: %08"HWADDR_PRIx" %zd bytes", i, addr, size);

        if (memory_region_test_and_clear_dirty(&d->vram_mr, addr, size,
                                               DIRTY_MEMORY_NV2A)) {
            NV20_VK_DPRINTF("Memory dirty. Synchronizing...");
...
void pgraph_vk_clear_surface(NV20State *d, uint32_t parameter)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;

    nv20_profile_inc_counter(NV20_PROF_CLEAR);
    ...
    NV20_VK_DGROUP_BEGIN("CLEAR min=(%d,%d) max=(%d,%d)%s%s", xmin, ymin, xmax,
...
#if 0
static void pgraph_vk_debug_attrs(NV20State *d)
{
    for (int vertex_idx = 0; vertex_idx < pg->draw_arrays_count[i]; vertex_idx++) {
        NV20_VK_DGROUP_BEGIN("Vertex %d+%d", pg->draw_arrays_start[i], vertex_idx);
        for (int attr_idx = 0; attr_idx < NV20_VERTEXSHADER_ATTRIBUTES; attr_idx++) {
            ...
            if (attr->count) {
                ...
                NV20_VK_DGROUP_BEGIN("Attribute %d data at %tx", attr_idx, (ptrdiff_t)(p - (char*)d->vram_ptr));
                for (int count_idx = 0; count_idx < attr->count; count_idx++) {
                    switch (attr->format) {
                    case NV097_SET_VERTEX_DATA_ARRAY_FORMAT_TYPE_F:
                        NV20_VK_DPRINTF("[%d] %f", count_idx, *(float*)p);
                        p += sizeof(float);
                        break;
                    default:
                        assert(0);
                               ...
                               VkDeviceSize offset)
{
    ...
    VkBuffer buffers[NV20_VERTEXSHADER_ATTRIBUTES];
    VkDeviceSize offsets[NV20_VERTEXSHADER_ATTRIBUTES];
...
{
    NV20_DPRINTF("pgraph_set_surface_dirty(%d, %d) -- %d %d\n", color, zeta,
...
typedef struct VertexBufferRemap {
    ...
    } map[NV20_VERTEXSHADER_ATTRIBUTES];
} VertexBufferRemap;
                                                    ...
                                                    uint32_t num_vertices)
{
    ...
    for (int attr_id = 0; attr_id < NV20_VERTEXSHADER_ATTRIBUTES; attr_id++) {
...
void pgraph_vk_flush_draw(NV20State *d)
{
    PGRAPHState *pg = &d->pgraph;
    PGRAPHVkState *r = pg->vk_renderer_state;

    if (!(r->color_binding || r->zeta_binding)) {
        NV20_VK_DPRINTF("No binding present!!!\n");
        return;
    }
    ...
    if (pg->draw_arrays_length) {
        NV20_VK_DGROUP_BEGIN("Draw Arrays");
        nv20_profile_inc_counter(NV20_PROF_DRAW_ARRAYS);
        ...
        for (int i = 0; i < pg->draw_arrays_length; i++) {
            ...
            NV20_VK_DPRINTF("- [%d] Start:%d Count:%d", i, start, count);
        ...
        NV20_VK_DGROUP_BEGIN("Inline Elements");
        assert(pg->inline_buffer_length == 0);
        assert(pg->inline_array_length == 0);

        nv20_profile_inc_counter(NV20_PROF_INLINE_ELEMENTS);
        ...
        NV20_VK_DGROUP_BEGIN("Inline Buffer");
        nv20_profile_inc_counter(NV20_PROF_INLINE_BUFFERS);
        assert(pg->inline_array_length == 0);

        size_t vertex_data_size = pg->inline_buffer_length * sizeof(float) * 4;
        void *data[NV20_VERTEXSHADER_ATTRIBUTES];
        size_t sizes[NV20_VERTEXSHADER_ATTRIBUTES];
        size_t offset = 0;
        ...
        NV20_VK_DGROUP_BEGIN("Inline Array");
        nv20_profile_inc_counter(NV20_PROF_INLINE_ARRAYS);
        ...
        for (int i = 0; i < NV20_VERTEXSHADER_ATTRIBUTES; i++) {
            ...
            NV20_DPRINTF("bind inline attribute %d size=%d, count=%d\n", i,
        ...
        NV20_DPRINTF("draw inline array %d, %d\n", vertex_size, index_count);
        ...
        NV20_VK_DPRINTF("EMPTY NV097_SET_BEGIN_END");
        NV20_UNCONFIRMED("EMPTY NV097_SET_BEGIN_END");
    }
}
