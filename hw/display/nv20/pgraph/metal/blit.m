...
void pgraph_metal_image_blit(NV20State *d)
{
    ...
    SurfaceBinding *surf_src = pgraph_metal_surface_get(d, source_addr);
    if (surf_src) {
    ...
    SurfaceBinding *surf_dest = pgraph_metal_surface_get(d, dest_addr);
    if (surf_dest) {
    ...
    NV20_DPRINTF("  blit 0x%tx -> 0x%tx (Size: %llu, Clipped Height: %zu)\n",
    ...
    memory_region_set_client_dirty(&d->vram_mr, dest_addr, clipped_dest_size,
                                   DIRTY_MEMORY_VGA);
    memory_region_set_client_dirty(&d->vram_mr, dest_addr, clipped_dest_size,
                                   DIRTY_MEMORY_VGA);
}
