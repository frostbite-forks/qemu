...
#ifndef HW_XBOX_NV20_PGRAPH_GLSL_VSH_H
#define HW_XBOX_NV20_PGRAPH_GLSL_VSH_H

#include "common.h"
#include "hw/display/nv20/pgraph/vsh_regs.h"
...
typedef struct FixedFunctionVshState {
    ...
    bool lighting;
    enum VshLight light[NV20_MAX_LIGHTS];
...
#define VSH_UNIFORM_DECL_X(S, DECL)                          \
    DECL(S, c, vec4, NV20_VERTEXSHADER_CONSTANTS)            \
    DECL(S, clipRange, vec4, 1)                              \
    DECL(S, fogParam, vec2, 1)                               \
    DECL(S, inlineValue, vec4, NV20_VERTEXSHADER_ATTRIBUTES) \
    DECL(S, lightInfiniteDirection, vec3, NV20_MAX_LIGHTS)   \
    DECL(S, lightInfiniteHalfVector, vec3, NV20_MAX_LIGHTS)  \
    DECL(S, lightLocalAttenuation, vec3, NV20_MAX_LIGHTS)    \
    DECL(S, lightLocalPosition, vec3, NV20_MAX_LIGHTS)       \
    DECL(S, ltc1, vec4, NV20_LTC1_COUNT)                     \
    DECL(S, ltctxa, vec4, NV20_LTCTXA_COUNT)                 \
    DECL(S, ltctxb, vec4, NV20_LTCTXB_COUNT)                 \
